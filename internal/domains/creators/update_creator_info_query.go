...
func UpdateCreatorInfo(ctx context.Context, neo neo4j.DriverWithContext, info UpdatedCreatorInfo, userId string) (UpdatedCreator, bool, int, err.Error) {
	status, aErr := auth.CheckAuthorizedUserCreator(ctx, neo, userId, info.Uid)
	if aErr.E != nil {
...
func buildUpdateCreatorInfoParams(info UpdatedCreatorInfo) map[string]any {
